...
// Fetch USDA data files from GitHub repo
async function fetchUSDAContext() {
  const files = [
    { name: 'cof', url: 'https://raw.githubusercontent.com/logancallen/cattlesignal/main/data/usda-cof.json' },
    { name: 'slaughter', url: 'https://raw.githubusercontent.com/logancallen/cattlesignal/main/data/usda-slaughter.json' },
    { name: 'exports', url: 'https://raw.githubusercontent.com/logancallen/cattlesignal/main/data/usda-exports.json' },
  ];
  const results = { cof: null, slaughter: null, exports: null };
  for (const f of files) {
    try {
      const res = await fetch(f.url, { signal: AbortSignal.timeout(5000) });
      if (!res.ok) continue;
      const data = await res.json();
      if (data.promptSnippet) results[f.name] = data.promptSnippet;
    } catch { /* skip */ }
  }
  return results;
}

function buildScoringPrompt(prices, cotData, usdaData) {
  ...
  const cotSection = cotData
    ? `\n${cotData}\nCOT_AVAILABLE: true`
    : `\nCOT POSITIONING DATA: NOT AVAILABLE. Score positioning as 50 (neutral).\nCOT_AVAILABLE: false`;

  const usdaCofSection = usdaData?.cof
    ? `\n${usdaData.cof}\nUSDA_COF_AVAILABLE: true`
    : `\nUSDA CATTLE ON FEED: NOT AVAILABLE. Note "Estimated without USDA data" for supply.\nUSDA_COF_AVAILABLE: false`;

  const usdaSlaughterSection = usdaData?.slaughter
    ? `\n${usdaData.slaughter}\nUSDA_SLAUGHTER_AVAILABLE: true`
    : `\nUSDA SLAUGHTER: NOT AVAILABLE. Note "Estimated without slaughter data" for supply.\nUSDA_SLAUGHTER_AVAILABLE: false`;

  const usdaExportsSection = usdaData?.exports
    ? `\n${usdaData.exports}\nUSDA_EXPORTS_AVAILABLE: true`
    : `\nUSDA EXPORTS: NOT AVAILABLE. Note "Estimated without export data" for demand.\nUSDA_EXPORTS_AVAILABLE: false`;

  const hasCot = !!cotData;
  const hasCof = !!usdaData?.cof;
  const hasSlaughter = !!usdaData?.slaughter;
  const hasExports = !!usdaData?.exports;
  const sourceCount = [true, hasCot, hasCof, hasSlaughter, hasExports].filter(Boolean).length;

  const dataSources = [
    'Price data: YES (15-min delayed Yahoo Finance)',
    `COT data: ${hasCot ? 'YES' : 'NO'}`,
    `USDA Cattle on Feed: ${hasCof ? 'YES' : 'NO'}`,
    `USDA Slaughter: ${hasSlaughter ? 'YES' : 'NO'}`,
    `USDA Exports: ${hasExports ? 'YES' : 'NO'}`,
    `Total sources: ${sourceCount}/5`,
  ].join('\n');
...
${cotSection}
${usdaCofSection}
${usdaSlaughterSection}
${usdaExportsSection}

CRITICAL RULES:
1. Only cite data you actually have above. Do NOT fabricate numbers you don't have.
2. If a data source is marked NOT AVAILABLE, note "Estimated without [source] data" in the factor note — do NOT score it as 50, instead make your best estimate based on available data and general knowledge, but flag it honestly.
3. Seasonal patterns must reflect the ACTUAL current month (${month}).
4. Confidence score must reflect data completeness:
   - ${sourceCount}/5 sources = confidence range ${Math.max(25, sourceCount * 18 - 5)}-${Math.min(98, sourceCount * 20)}
5. Factor notes must be under 15 words. Cite real numbers from the data when available.
...
1. supply: Supply fundamentals based on available USDA and market data
2. demand: Demand strength including export data if available
3. positioning: COT/managed money (use available COT data, or estimate if unavailable)
...
async function generateScores() {
  ...
  try {
    // Try to get COT and USDA data
    const [cotData, usdaData] = await Promise.all([
      fetchCOTContext(),
      fetchUSDAContext(),
    ]);
    console.log(`  COT: ${cotData ? 'available' : 'unavailable'}`);
    console.log(`  USDA COF: ${usdaData.cof ? 'available' : 'unavailable'}`);
    console.log(`  USDA Slaughter: ${usdaData.slaughter ? 'available' : 'unavailable'}`);
    console.log(`  USDA Exports: ${usdaData.exports ? 'available' : 'unavailable'}`);

    const prompt = buildScoringPrompt(priceCache.prices, cotData, usdaData);
