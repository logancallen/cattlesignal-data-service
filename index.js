...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}html,body,#root{height:100%}body{font-family:var(--font);background:var(--bg);color:var(--text);-webkit-font-smoothing:antialiased}input,select,button,textarea{font-family:inherit}select{appearance:auto}::selection{background:var(--blue);color:#fff}input:focus,select:focus{outline:1px solid var(--blue);outline-offset:-1px}button{transition:all .15s ease;cursor:pointer}button:hover{filter:brightness(1.12)}button:active{transform:scale(.98)}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px}
    ...
    @media(max-width:900px){.two-col{grid-template-columns:1fr!important}.result-grid,.rr-grid{grid-template-columns:1fr!important}.preset-grid{grid-template-columns:1fr 1fr!important}.acct-grid{grid-template-columns:1fr 1fr!important}.acct-bar-inner{flex-direction:column!important;align-items:stretch!important}.nav-inner{padding:8px 14px!important}.nav-tabs{overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch}.score-grid-3{grid-template-columns:1fr 1fr!important}.score-grid-4{grid-template-columns:1fr 1fr!important}}
...
const PRICES_URL="https://cattlesignal-data-service-production.up.railway.app/prices";
const scoreColor=(v)=>{
  if(v<=15)return"#8B0000";if(v<=25)return"#B91C1C";if(v<=35)return"#DC143C";
  if(v<=42)return"#E85D5D";if(v<=48)return"#D4956A";if(v<=52)return"#C4A535";
  if(v<=58)return"#A3B236";if(v<=65)return"#7DB358";if(v<=75)return"#3DAA5C";
  if(v<=85)return"#1E8C3E";if(v<=92)return"#167A34";return"#0D6E2B";
};
const confLabel=(v)=>v>=70?"High":v>=45?"Medium":"Low";
const confColor=(v)=>v>=70?"var(--green)":v>=45?"var(--gold)":"var(--red)";
/* Simple markdown: **bold**, \n → <br> */
function renderMd(t){if(!t)return"";return t.replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\n/g,"<br/>")}
...
function Scorecard(){
  ...
  const fetchScores=async()=>{
    ...
    try{
      ...
      else if(d.lastError)setErr("Scoring error: "+d.lastError);
  ...
  const updatedText=data?.generatedAt?(()=>{const m=Math.round((Date.now()-new Date(data.generatedAt).getTime())/60000);return m<1?"Just now":m+"m ago"})():"";
  const ds=data?.dataSources||{};

  const ScoreCell=({s,wide})=>{
    const bg=scoreColor(s.composite);
    const textCol=s.composite>40&&s.composite<60?"#1a1a1a":"#fff";
    const showConf=s.confidence<70; // Only show badge if Medium or Low
    return(
      <div onClick={()=>setSelected(selected===s.code?null:s.code)} style={{
        ...
        border:selected===s.code?`2px solid #fff`:"2px solid transparent",
        boxShadow:selected===s.code?`0 0 24px ${bg}50`:"none",
        transform:selected===s.code?"scale(1.02)":"scale(1)",
        opacity:loading&&data?.scores?.length?0.7:1,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          ...
          {showConf&&<div style={{fontSize:8,fontWeight:600,color:s.confidence<45?"#fca5a5":"#fcd34d",padding:"2px 5px",borderRadius:3,background:"rgba(0,0,0,.3)"}}>{confLabel(s.confidence)}</div>}
        </div>
        <div style={{display:"flex",alignItems:"baseline",gap:4}}>
          ...
          <div style={{fontSize:11,fontWeight:600,color:textCol,opacity:.5,fontFamily:"var(--mono)"}}>/100</div>
        </div>
        ...
        <div style={{marginTop:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{fontSize:10,fontFamily:"var(--mono)",color:textCol,opacity:.8,fontWeight:600}}>{s.changePct>=0?"+":""}{s.changePct?.toFixed(2)||0}%</div>
        </div>
      </div>
    );
  };
  ...
  const FactorBar=({label,score,note})=>(
    <div style={{marginBottom:10}}>
      ...
      <div style={{height:6,background:"var(--card2)",borderRadius:3,overflow:"hidden",border:"1px solid var(--border)"}}>
        <div style={{height:"100%",width:score+"%",background:`linear-gradient(90deg, ${scoreColor(Math.max(0,score-15))}, ${scoreColor(score)})`,borderRadius:3,transition:"width .6s ease"}}/>
      </div>
      <div style={{fontSize:9,color:note?.includes("unavailable")?"var(--gold)":"var(--text3)",marginTop:2,fontStyle:note?.includes("unavailable")?"italic":"normal"}}>{note}</div>
    </div>
  );
    ...
    <div className="fade-up" style={{maxWidth:960,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
        <div>
          ...
          <div style={{fontSize:10,color:"var(--text3)",marginTop:2}}>
            AI-generated outlook scores · Educational analysis only · {updatedText&&`Updated ${updatedText}`}
          </div>
        </div>
      ...
      {data?.dataSources&&(
        <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
          {[["Prices",ds.prices],["COT",ds.cot],["USDA",ds.usda],["Slaughter",ds.slaughter],["Exports",ds.exports]].map(([name,available])=>(
            <div key={name} style={{fontSize:8,fontWeight:700,padding:"2px 6px",borderRadius:3,background:available?"var(--greenbg)":"var(--card2)",color:available?"var(--green)":"var(--text3)",border:`1px solid ${available?"var(--greenborder)":"var(--border)"}`}}>{available?"✓":"✗"} {name}</div>
          ))}
        </div>
      )}
      ...
      {data?.scores&&(<>
        ...
        <div style={{marginBottom:6}}>
          ...
          <div className="score-grid-3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
        ...
        <div style={{marginBottom:6}}>
          ...
          <div className="score-grid-4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
            {grains.map(s=><ScoreCell key={s.code} s={s}/>)}
          </div>
        </div>

        {/* Energy — full width single card */}
        <div style={{marginBottom:6}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {energy.map(s=><ScoreCell key={s.code} s={s} wide/>)}
          </div>
        </div>
        ...
        {sel&&(
          <div className="fade-up" style={{marginTop:16,background:"var(--card)",border:"1px solid var(--border)",borderRadius:12,overflow:"hidden"}}>
            <div style={{padding:"16px 20px",borderBottom:"1px solid var(--border)",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              ...
              <button onClick={()=>setSelected(null)} style={{marginLeft:12,padding:"4px 8px",borderRadius:4,fontSize:10,fontWeight:600,background:"var(--card2)",color:"var(--text3)",border:"1px solid var(--border)",cursor:"pointer"}}>✕</button>
            </div>
        ...
        <div style={{marginTop:16}}>
          <button onClick={()=>setShowMethod(!showMethod)} style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:8,padding:"10px 14px",width:"100%",textAlign:"left",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:11,fontWeight:700,color:"var(--text2)"}}>How Scores Work</span>
            <span style={{fontSize:10,color:"var(--text3)"}}>{showMethod?"▲":"▼"}</span>
          </button>
          {showMethod&&(
            <div className="fade-up" style={{background:"var(--card)",border:"1px solid var(--border)",borderTop:"none",borderRadius:"0 0 8px 8px",padding:"14px 16px",fontSize:11,color:"var(--text2)",lineHeight:1.8}}>
              <p style={{marginBottom:8}}>Each market receives a <strong style={{color:"var(--text)"}}>composite outlook score (0-100)</strong> derived from 5 weighted factors:</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 16px",fontSize:10,marginBottom:8}}>
                <div><strong style={{color:"var(--text)"}}>Supply (25%)</strong> — Inventory, production, imports</div>
                <div><strong style={{color:"var(--text)"}}>Demand (20%)</strong> — Consumption, exports, seasonal</div>
                <div><strong style={{color:"var(--text)"}}>Positioning (15%)</strong> — CFTC COT managed money</div>
                <div><strong style={{color:"var(--text)"}}>Macro (15%)</strong> — Rates, energy, trade policy</div>
                <div><strong style={{color:"var(--text)"}}>Seasonal (15%)</strong> — Historical monthly patterns</div>
                <div><strong style={{color:"var(--text)"}}>Discretionary (10%)</strong> — Unique current factors</div>
              </div>
              <p style={{marginBottom:8}}><strong style={{color:"var(--text)"}}>Confidence scores</strong> reflect data completeness. More data sources = higher confidence. Check the badges at the top of the scorecard to see which sources are currently active. Full integration of all 5 sources (prices, COT, USDA Cattle on Feed, slaughter, and export sales) enables 80-95% confidence.</p>
              <p style={{marginBottom:0,fontSize:10,color:"var(--text3)"}}>Scores update every 15 minutes when users are active. No AI calls are made when nobody is using the site. Scores are smoothed to prevent erratic swings — maximum 15-point change per refresh unless a major catalyst is detected.</p>
            </div>
          )}
        </div>

        {/* Bottom Disclaimer */}
        <div style={{marginTop:12,padding:"10px 14px",background:"var(--card)",borderRadius:8,border:"1px solid var(--border)"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {aiMsgs.map((m,i)=>(
                <div key={i} className="fade-up" style={{background:"var(--card)",borderRadius:8,border:"1px solid var(--border)",padding:"12px 14px",borderLeft:m.r==="ai"?"2px solid var(--blue)":"2px solid transparent"}}>
                  ...
                  <div style={{fontSize:12,lineHeight:1.8,whiteSpace:"pre-wrap"}} dangerouslySetInnerHTML={{__html:renderMd(m.t)}}></div>
                </div>
              ))}
